...
ModelerControl::ModelerControl()
    : m_minimum(0.0f), m_maximum(1.0f), m_stepsize(0.1f), m_value(0.0f) {}

ModelerControl::ModelerControl(const char *name, float minimum, float maximum,
...
void ModelerControl::SetVals(const char *name, float minimum, float maximum,
                             float stepsize, float value) {
...
void ModelerApplication::Init(ModelerViewCreator_f createView,
  ...
  Fl::background(GetRValue(dwBtnFaceColor), GetGValue(dwBtnFaceColor),
  ...
  for (i = 0; i < m_numControls; i++) {
    m_ui->addControl(controls[i].m_name, controls[i].m_minimum,
                     controls[i].m_maximum, controls[i].m_stepsize,
                     controls[i].m_value);
  }
