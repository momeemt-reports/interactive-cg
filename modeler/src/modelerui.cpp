...
const char *ModelerUI::fileDialog(Fl_Native_File_Chooser::Type dialogType,
...
inline void ModelerUI::cb_openAniScript_i(Fl_Menu_ *, void *) {
  const char *szFileName =
      fileDialog(Fl_Native_File_Chooser::BROWSE_FILE,
                 "Animation Script (*.ani)\t*.ani", "Open Animation Script");
...
inline void ModelerUI::cb_saveAniScript_i(Fl_Menu_ *, void *) {
  const char *szFileName =
      fileDialog(Fl_Native_File_Chooser::BROWSE_SAVE_FILE,
                 "Animation Script (*.ani)\t*.ani", "Save Animation Script");
  if (!szFileName) // blank file name
    return;        // do nothing
...
inline void ModelerUI::cb_saveImageFile_i(Fl_Menu_ *, void *) {
  const char *szFilename = fileDialog(
  ...
  m_pwndModelerView->saveImage(szFilename, filetype == 0 ? ".png" : ".jpg",
                               quality);
...
inline void ModelerUI::cb_saveMovie_i(Fl_Menu_ *, void *) {
  ...
  if (!stricmp(szExt, ".png") || !stricmp(szExt, ".jpg")) {
    m_strMovieFileName =
        m_strMovieFileName.substr(0, m_strMovieFileName.length() - 4);
    ext = (string)szExt;
  }
#endif
  quality = 95;
  ...
  for (int i = 0; i < m_iCurrControlCount; ++i) {
    if (m_pbrsBrowser->selected(i + 1)) {
      ...
      if (iSelectedIndex == FL_YELLOW)
        ++iSelectedIndex; // can't use yellow because FLTK will change it to
                          // black
    } else {
      labelBox(i)->hide();
      valueSlider(i)->hide();
...
void ModelerUI::redrawRulers() {
  m_pwndHRuler->range(m_pwndGraphWidget->leftTime(),
                      m_pwndGraphWidget->rightTime());
  m_pwndHRuler->redraw();
  m_pwndVRuler->rulercolor(m_pwndGraphWidget->currCurveColor());
  m_pwndVRuler->range(m_pwndGraphWidget->bottomValue(),
                      m_pwndGraphWidget->topValue());
  m_pwndVRuler->redraw();
}
...
void ModelerUI::currTime(float fTime) {
  ...
  */
  ...
  if (simulate()) {
    ...
    if (ps != NULL) {
      float bakeStartTime = ModelerApplication::Instance()
                                ->GetParticleSystem()
                                ->getBakeStartTime();
      float bakeEndTime =
          ModelerApplication::Instance()->GetParticleSystem()->getBakeEndTime();
      if (bakeEndTime < 0.0f)
        bakeEndTime = fTime;
...
float ModelerUI::controlValue(int iControl) const {
  ...
  if (m_ptabTab->value() != (Fl_Widget *)m_pgrpCurveGroup) {
    ...
    // curve mode
    return m_pwndGraphWidget->curve(iControl)->evaluateCurveAt(
        m_pwndGraphWidget->currTime());
  }
}
...
ModelerUI::ModelerUI()
    : m_iCurrControlCount(0), m_pcbfValueChangedCallback(NULL), m_iFps(30),
      m_bAnimating(false), m_bSaveMovie(false), m_nativeChooser(NULL) {
...
void ModelerUI::show() {
  ...
  if (userResolution) {
    m_pwndModelerWnd->resize(40 + m_pwndMainWnd->w(), 30, ModelerUI::userWidth,
                             ModelerUI::userHeight);
    m_pwndModelerWnd->show();
  }
  // else use the default resolution
  else {
    m_pwndModelerWnd->resize(40 + m_pwndMainWnd->w(), 30, m_pwndModelerWnd->w(),
                             m_pwndModelerWnd->h());
    m_pwndModelerWnd->show();
  }
...
void ModelerUI::addControl(const char *szName, float fMin, float fMax,
                           float fStepSize, float fInitVal) {
  ...
  Fl_Value_Slider *slider =
      new Fl_Value_Slider(0, 0, m_ppckPack->w(), k_iSliderHeight, 0);
  slider->type(1);
  slider->hide();
...
bool ModelerUI::openAniScript(const char *szFileName) {
  ...
  // for (int ikf = 0; ikf < m_pwndModelerView->m_curve_camera->numKeyframes();
  // ++ikf)
