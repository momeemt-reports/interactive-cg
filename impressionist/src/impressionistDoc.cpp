...
ImpressionistDoc::ImpressionistDoc() {
  ...
  ImpBrush::c_pBrushes[BRUSH_SCATTERED_CIRCLES] =
      new ScatteredCircleBrush(this, "ScatteredCircles");
  ImpBrush::c_pBrushes[BRUSH_VIVID_POINTS] =
      new VividPointBrush(this, "VividPoints");
  ImpBrush::c_pBrushes[BRUSH_VIVID_SCATTERED_LINES] =
      new VividScatteredLineBrush(this, "VividScatteredLines");
 ...
 *such that origImg[3*(row*srcBufferWidth+column)+0],
 *							origImg[3*(row*srcBufferWidth+column)+1],
 *							origImg[3*(row*srcBufferWidth+column)+2]
 *							are R, G, B values for
 *pixel at (column, row). destBuffer:			the image buffer to put
 *the resulting image in.  It is always the same size as the source buffer.
 *
 ...
 *							divided by divisor and
 *then added by offset
 */
void ImpressionistDoc::applyFilter(const unsigned char *sourceBuffer,
                                   int srcBufferWidth, int srcBufferHeight,
                                   unsigned char *destBuffer,
                                   const double *filterKernel, int knlWidth,
                                   int knlHeight, double divisor,
                                   double offset) {
