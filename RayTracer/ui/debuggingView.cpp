...
DebuggingView::DebuggingView(int x, int y, int w, int h, char *label)
    : Fl_Gl_Window(x, y, w, h, label), raytracer(0), m_shadingMode(NORMAL),
      m_quality(HIGH_QUALITY), m_lightingMode(DEFAULT_LIGHTING),
...
void DebuggingView::resetCamera() {
  ...
  maxVec =
      maximum(scene.getCamera().getEye() + scene.getCamera().getLook(), maxVec);
...
void DebuggingView::lightScene() {
  ...
  // Reset all the lights
  {
    ...
    for (std::vector<GLenum>::const_iterator glLightsItr = lights.begin();
         glLightsItr != lights.end(); ++glLightsItr) {
  ...
  if (m_lightingMode == SCENE_LIGHTING) {
    ...
    for (std::vector<Light *>::const_iterator l =
             raytracer->getScene().beginLights();
         l != raytracer->getScene().endLights(); ++l) {
      if (glLightsItr == lights.end())
        std::cerr << "Not displaying all lights in debugging mode; GL can only "
                     "handle 8."
                  << std::endl;
...
void DebuggingView::drawCamera() {
  ...
  glVertex3dv(
      (-0.5 * sceneCamera.getU() - 0.5 * sceneCamera.getV()).getPointer());
  ...
  glVertex3dv(
      (-0.51 * sceneCamera.getU() - 0.51 * sceneCamera.getV()).getPointer());
  glVertex3dv(
      (-0.51 * sceneCamera.getU() + 0.51 * sceneCamera.getV()).getPointer());
  glVertex3dv(
      (0.51 * sceneCamera.getU() + 0.51 * sceneCamera.getV()).getPointer());
